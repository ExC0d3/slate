...
SettingsDialog.prototype.saveSettings = function( e ){
  ...
  if( connection ){
    ...
    this.displaySaved( form.parentNode.parentNode );
...
SettingsDialog.prototype.show = function(){
  ...
  el.on('click', '#save-all-connection', function(e){
    e.preventDefault();
  ...
  el.on('click', '.add-channel', function(e){
    ...
    //this.saveSettings(e.target.form);
    self.addChannel(settings.connections[index]);
  }.bind(this));

  el.on('click', '.remove-channel', function(e){
    e.preventDefault();
    var form = e.target.form;

    // remove list item containing the channel
 ...
 * @api private
 */
SettingsDialog.prototype.saveAllConnections = function( suppressAlert ){
  var connections = [],
      connection;
  this.el.find( 'form' ).each(function( form ){
    connection = this.getConnectionSettings( form );
    // test if connection object is returned
    if( !connection ){
      return;
    }
    connections.push( this.getConnectionSettings( form ) );
    // test if display the saved confirmation
    if( !suppressAlert ){
      this.displaySaved( form.parent('li').els[0] );
    }
  }.bind(this));
  // update
...
/**
 * add a className to the connection pannel in order to display a confirmation
 *
 * @api private
 */
SettingsDialog.prototype.displaySaved = function( connection ){
  connection.classList.add( 'saved' );
  (function(conn){
    window.setTimeout( function(){
      conn.classList.remove('saved');
    }, 1500 );
  })(connection);
}